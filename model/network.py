...
class Network(object):
    ...
    def calc_loss(self, logits, y_true):
        ...
        object_angle = y_true[:, :, :, :, 4:6]
        object_masks = y_true[:, :, :, :, 6:7]
        object_probs = y_true[:, :, :, :, 7:]
        ...
        pred_box_xy, pred_box_wh, pred_box_remi = self.reorg_layer(logits, self.anchors)

        predictions = tf.reshape(logits, [-1, feature_size[0], feature_size[1], self.anchor_num, self.class_num + 7])
        pred_conf_logits = predictions[:, :, :, :, 6:7]
        pred_prob_logits = predictions[:, :, :, :, 7:]
